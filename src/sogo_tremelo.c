...
// --- BUTTON STATE ---
volatile uint16_t button_state    = 0;  // bitmask of current state
volatile uint16_t button_pressed  = 0;  // newly pressed
volatile uint16_t button_released = 0;  // newly released
...
//tremelo variables
static volatile float tremolo_depth = 0.0f;   // 0.0 = off, 1.0 = max
static volatile float tremolo_phase = 0.0f;
#define TREMOLO_STEP 0.2f
#define TREMOLO_RATE 6.0f  // Hz — tweak to taste
...
// -----------------------------
static int semitones_from_valves(uint16_t state) {
    // bits 0,1,2 = valves 1,2,3
    uint16_t valves = (state >> 4) & 0x0F;
...
bool timer_callback(struct repeating_timer *t) {
    ...
    if (adc_hw->result < THRESHOLD_OFF) {
        playing = false;
        tremolo_depth = 0.0f;   // reset tremolo on breath-off
        tremolo_phase = 0.0f;
    ...
    if (++trem_counter >= 44) {  // update tremolo ~1000 times per second instead of 44100
    trem_counter = 0;
    tremolo_phase += TREMOLO_RATE / 1000.0f;
    if (tremolo_phase > 1.0f) tremolo_phase -= 1.0f;
    trem_gain = 1.0f - tremolo_depth * 1.0f * (tremolo_phase < 0.5f ? tremolo_phase * 2.0f : (1.0f - tremolo_phase) * 2.0f);
}
...
int main() {
    ...
    wav_sample_rate = info.sample_rate;
    ...
    while (1) {
        ...
        printf("adc: %d playing: %d pitch: %.4f\n", adc_hw->result, playing, pitch_mult);  
