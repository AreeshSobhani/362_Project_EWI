...
uint16_t vibrato_init(uint16_t input_sample, uint32_t sample_rate);
...
volatile uint16_t potentiometer = 0; //value of the potentiometer 
volatile uint16_t mouthpiece = 0; //value of the mouthpiece

//vibrato state
float vib_phase = 0.0f; 
float vib_rate = 5.5f; //natural vibrato
...
void play_wav_from_file(FIL *fil, uint16_t bytes_per_frame, uint32_t timer_us, uint32_t sample_rate) {
    ...
    while (true) {
        ...
        if (sample_ready) {
            ...
            uint16_t vib_sample = vibrato_init(next_sample, sample_rate); 
            dac_write_scaled(vib_sample); 
...
void init_mouthpiece()
{
    ...
    //config for potentiometer value
    adc_gpio_init(44); 
    adc_select_input(4); 
...
uint16_t vibrato_init(uint16_t input_sample, uint32_t sample_rate)
{
    //map potentionmeter and depth
    float depth = 0.0f;

    if (potentiometer > 10) {
        depth = (float)(potentiometer - 10) / (4095.0f - 10.0f); //normalize 0-1
    }

    //update the low-frequency oscillator
    vib_phase += 2.0f * M_PI * vib_rate / (float)sample_rate;
    if (vib_phase > 2.0f * M_PI)
        vib_phase -= 2.0f * M_PI;

    float vib = sinf(vib_phase);

    //apply modulation
    float max_depth = 0.05f;   
    float mod = 1.0f + vib * depth * max_depth;

    // convert to signed
    int32_t signed_sample = (int32_t)input_sample - 0x8000;

    // apply modulation
    int32_t out = (int32_t)(signed_sample * mod);

    // back to unsigned
    return (uint16_t)(out + 0x8000);
}
...
void meter_out()
{
    ...
    else if (mouthpiece <= 0)
    {
        level = 0.0f;
    }
    else
    {
        level = (float) (mouthpiece)/4095;
    }
...
int main()
{
    ...
    for (;;) {
        ...
        { 
            ...
            if (mouthpiece > THRESHOLD_ON && !playing) {
                f_lseek(&fil, data_start);
                play_wav_from_file(&fil, bytes_per_frame, timer_us, sample_rate);
            }
    }
